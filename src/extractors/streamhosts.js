...
async function resolveEmbedAdvanced(rawUrl, fallbackReferer, opts = {}) {
  ...
  const out = {
    ...
    label,
    via,
    // Los masters .txt no terminan en .m3u8: el player no los reconoce como HLS
    // si se le da la URL cruda, así que se sirven por el proxy liviano.
    lightProxy: !isMp4 && /\.txt(\?|#|$)/i.test(r.url),
  };
